 ...
 */function parseHash(a){if(debug("parsing hash",a),!a)return new Map;// strip leading "#" or "#/"
const b=a.replace(/^#\/?/,"").split("&").map(function(a){return a.split("=",2)});return new Map(b)}function unwrapExports(a){return a&&a.__esModule&&Object.prototype.hasOwnProperty.call(a,"default")?a["default"]:a}function createCommonjsModule(a,b){return b={exports:{}},a(b,b.exports),b.exports}/*!
 * cookie
 ...
 *    See the License for the specific language governing permissions and
 *    limitations under the License.
 */const cookie$1=new lib_2;class StorageHandler{saveOAuthState(a,b){cookie$1.setItem(getKey(a),JSON.stringify(b))}getOAuthState(a){const b=cookie$1.getItem(getKey(a));return b?JSON.parse(b):null}clearOAuthState(a){cookie$1.removeItem(getKey(a))}saveSessionState(a,b){storage.setItem(getSessionKey(a),JSON.stringify(b))}getSessionState(a){const b=getSessionKey(a),c=storage.getItem(b);return c?JSON.parse(c):null}clearSessionState(a){storage.removeItem(getSessionKey(a))}}function getKey(a){return"oauth-state-"+encodeURIComponent(a)}function getSessionKey(a){return getKey(a)+"-active-session"}/*
 *  @license
 *    Copyright 2018 Brigham Young University
 *
 *    Licensed under the Apache License, Version 2.0 (the "License");
 ...
 *    See the License for the specific language governing permissions and
 *    limitations under the License.
 */let SINGLETON_INSTANCE;const CHILD_IFRAME_ID="byu-oauth-implicit-grant-refresh-iframe",FIFTY_FIVE_MINUTES_MILLIS=33e5,STORED_STATE_LIFETIME=300000,IG_STATE_AUTO_REFRESH_FAILED="implicit-grant-auto-refresh-failed";class ImplicitGrantProvider{constructor(a,b,c,d=new StorageHandler){// strip trailing slash(es)
debug("initializing provider with config",a),this.config=a,this.window=b,this.document=c,this.storageHandler=d,this._listeners={},this.config.baseUrl=this.config.issuer.replace(/\/+$/,""),this.store=Object.freeze({state:STATE_INDETERMINATE,user:null,token:null,error:null}),debug("initialized provider")}_changeState(a,b,c,d){logStateChange(a,b,c,d),this.store=Object.freeze({state:a,user:b,token:c,error:d}),_dispatchEvent(this,EVENT_STATE_CHANGE,this.store)}_checkPopupOpener(){try{const a=this.window.opener.location.origin;if(a===new URL(this.config.callbackUrl).origin)// Origins match
...
return f?d?void 0:(debug("dispatching event to parent"),_dispatchEvent(this.window.parent,EVENT_STATE_CHANGE,{state:a,token:c,user:b,source:"iframe"}),void(a===STATE_AUTHENTICATED&&(info("removing child iframe"),f.parentNode.removeChild(f)))):void(this._maybeUpdateStoredSession(a,b,c),a===STATE_AUTHENTICATED&&this._checkExpired(c.expiresAt.getTime()))}async startup(){ensureOnlyInstance(this),info("starting up"),this.listen(),this._changeState(STATE_INDETERMINATE);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){debug("handling authentication callback"),this._changeState(STATE_AUTHENTICATING);try{const{state:c,user:d,token:e,error:f}=await _handleAuthenticationCallback(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){error("OAuth Error",a),this._changeState(STATE_ERROR,void 0,void 0,a)}}else this.hasStoredSession()?(debug("Has stored session"),this._updateStateFromStorage()):(debug("no authentication present"),this._changeState(STATE_UNAUTHENTICATED));return this}_checkExpired(a){debug("checking expiration time");const b=a-Date.now(),c=0>b,d=b>FIFTY_FIVE_MINUTES_MILLIS&&!this._alreadyDidFunkyCheck;return this._alreadyDidFunkyCheck=!c,c||d?void(this.config.autoRefreshOnTimeout?(d&&debug("silently refreshing token to work around odd identity server issue"),this._changeState(STATE_REFRESHING,this.store.user,this.store.token),this._scheduleRefresh()):c&&this._changeState(STATE_EXPIRED,this.store.user,this.store.token)):void this._scheduleExpirationCheck(a)}_scheduleRefresh(){var a=this;return info("scheduling auto-refresh"),this.__refreshTask?void debug("refresh already scheduled"):this.__refreshTask=setTimeout(function(){a.__refreshTask=null,a.startRefresh("iframe")},5e3);// Wait a few seconds before triggering the actual refresh, allowing for clock
// skew with WSO2
...
.sort(function(c,a){return a.length-c.length}).find(function(a){return b.href.startsWith(a)});if(d)return Object.assign({callbackUrl:d},a[d]);throw new Error(`Unable to match url [${b.href}] to one of [${c}]`)}export{DEFAULT_ISSUER,GLOBAL_CONFIG_KEY,IG_STATE_AUTO_REFRESH_FAILED,configure};
