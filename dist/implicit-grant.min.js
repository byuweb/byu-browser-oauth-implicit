...
const e=this.window.parent.document.getElementById(CHILD_IFRAME_ID);return e?d?void 0:(debug("dispatching event to parent"),_dispatchEvent(this.window.parent,EVENT_STATE_CHANGE,{state:a,token:c,user:b,source:"iframe"}),void(a===STATE_AUTHENTICATED&&(info("removing child iframe"),e.parentNode.removeChild(e)))):void(this._maybeUpdateStoredSession(a,b,c),a===STATE_AUTHENTICATED&&this._checkExpired(c.expiresAt.getTime()))}async startup(){info("starting up"),this.listen(),this._changeState(STATE_INDETERMINATE);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){debug("handling authentication callback"),this._changeState(STATE_AUTHENTICATING);try{const{state:c,user:d,token:e,error:f}=await _handleAuthenticationCallback(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){error("OAuth Error",a),this._changeState(STATE_ERROR,void 0,void 0,a)}}this.hasStoredSession()?(debug("Has stored session"),this._updateStateFromStorage()):(debug("no authentication present"),this._changeState(STATE_UNAUTHENTICATED))}_checkExpired(a){var b=this;debug("checking expiration time");const c=a-Date.now(),d=0>c,e=c>FIFTY_FIVE_MINUTES_MILLIS;return d||e?void(this.config.autoRefreshOnTimeout?(e&&debug("silently refreshing token to work around odd identity server issue"),this._changeState(STATE_REFRESHING,this.store.user,this.store.token),this._schedulePeriodic(function(){return b.startRefresh("iframe")})):d&&this._changeState(STATE_EXPIRED,this.store.user,this.store.token)):void this._scheduleExpirationCheck(a)}_scheduleRefresh(){var a=this;return info("scheduling auto-refresh"),this.__refreshTask?void debug("refresh already scheduled"):this.__refreshTask=this._schedulePeriodic(function(){null==a.__refreshTask,a.startRefresh("iframe")})}_scheduleExpirationCheck(a){var b=this;return this.__expirationTask&&cancelTimeout(this.__expirationTask),this.__expirationTask=this._schedulePeriodic(function(){b.__expirationTask=null,b._checkExpired(a)})}_schedulePeriodic(a){// Simply using setTimeout for an hour in the future
...
error:d}];d?error(...e):info(...e)}function redactUser(a){return a?{netId:a.netId,"rest-is-redacted":!0}:void 0}function redactToken(a){if(console.log("redacting token",a),!!a){console.log(a);const{bearer:b,expiresAt:c,client:d}=a;return console.log(c),console.log(typeof c),{bearer:redactBearerToken(b),expiresAt:c?c.toISOString():null,client:d,"rest-is-redacted":!0}}}function redactBearerToken(a){return a?a.substring(0,2)+"...redacted..."+a.substring(a.length-2):void 0}/*
 * Copyright 2018 Brigham Young University
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
