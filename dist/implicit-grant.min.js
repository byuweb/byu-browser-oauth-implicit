...
if(this.window.opener)// We're inside a child re-authentication popup
return d?void 0:(_dispatchEvent(this.window.opener,EVENT_STATE_CHANGE,{state:a,token:c,user:b,source:"popup"}),void(a===STATE_AUTHENTICATED&&this.window.close()));// Pass event along to parent
// If we're inside the "refresh" iframe
const e=this.window.parent.document.getElementById(CHILD_IFRAME_ID);return e?d?void 0:(_dispatchEvent(this.window.parent,EVENT_STATE_CHANGE,{state:a,token:c,user:b,source:"iframe"}),void(a===STATE_AUTHENTICATED&&e.parentNode.removeChild(e))):void(this._maybeUpdateStoredSession(a,b,c),a===STATE_AUTHENTICATED&&this._checkRefresh(c.expiresAt.getTime()))}async startup(){this.listen(),this._changeState(STATE_INDETERMINATE);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){this._changeState(STATE_AUTHENTICATING);try{const{state:c,user:d,token:e,error:f}=await _handleAuthenticationCallback(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){console.error("OAuth Error",a),this._changeState(STATE_ERROR,void 0,void 0,a)}}this.hasStoredSession()?this._updateStateFromStorage():this._changeState(STATE_UNAUTHENTICATED)}_checkRefresh(a){var b=this;// Simply using setTimeout for an hour in the future
