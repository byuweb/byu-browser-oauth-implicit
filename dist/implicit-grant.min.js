const LEVEL_DEBUG={priority:0,name:"debug"},LEVEL_INFO={priority:10,name:"info"},LEVEL_ERROR={priority:100,name:"error"},ALL_LEVELS=[LEVEL_DEBUG,LEVEL_INFO,LEVEL_ERROR],DEFAULT_LEVEL=LEVEL_INFO;function debug(...a){doLog(LEVEL_DEBUG,...a)}function info(...a){doLog(LEVEL_INFO,...a)}function error(...a){doLog(LEVEL_ERROR,...a)}function doLog(a,...b){if(shouldLog(a)){const c=new Date().toLocaleTimeString({h12:!1});console.log("[byu-browser-oauth-implicit]",`[${a.name}]`,`(${c})`,...b)}}function shouldLog(a){return a.priority>=currentLevel().priority}function currentLevel(){const a=levelAttr()||levelGlobalVar();if(!a)return DEFAULT_LEVEL;const b=a.toLowerCase();return ALL_LEVELS.find(function(a){return a.name===b})||DEFAULT_LEVEL}function levelAttr(){return document.documentElement.getAttribute("byu-oauth-logging")}function levelGlobalVar(){const a=window.byuOAuth||{};return a.logging}const EVENT_PREFIX="byu-browser-oauth",EVENT_STATE_CHANGE=`${"byu-browser-oauth"}-state-changed`,EVENT_LOGIN_REQUESTED=`${"byu-browser-oauth"}-login-requested`,EVENT_LOGOUT_REQUESTED=`${"byu-browser-oauth"}-logout-requested`,EVENT_REFRESH_REQUESTED=`${"byu-browser-oauth"}-refresh-requested`,EVENT_CURRENT_INFO_REQUESTED=`${"byu-browser-oauth"}-current-info-requested`,STATE_INDETERMINATE="indeterminate",STATE_UNAUTHENTICATED="unauthenticated",STATE_AUTHENTICATED="authenticated",STATE_AUTHENTICATING="authenticating",STATE_ERROR="error";/*
 *  @license
...
const e=parent.document.getElementById(CHILD_IFRAME_ID);return e?d?void 0:(debug("dispatching event to parent"),_dispatchEvent(parent,EVENT_STATE_CHANGE,{state:a,token:c,user:b,source:"iframe"}),void(a===STATE_AUTHENTICATED&&(info("removing child iframe"),e.parentNode.removeChild(e)))):void(this._maybeUpdateStoredSession(a,b,c),a===STATE_AUTHENTICATED&&this._checkRefresh(c.expiresAt.getTime()))}async startup(){info("starting up"),this.listen(),this._changeState(STATE_INDETERMINATE);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){debug("handling authentication callback"),this._changeState(STATE_AUTHENTICATING);try{const{state:c,user:d,token:e,error:f}=await _handleAuthenticationCallback(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){error("OAuth Error",a),this._changeState(STATE_ERROR,void 0,void 0,a)}}this.hasStoredSession()?(debug("Has stored session"),this._updateStateFromStorage()):(debug("no authentication present"),this._changeState(STATE_UNAUTHENTICATED))}_checkRefresh(a){var b=this;debug("checking expiration time");// Simply using setTimeout for an hour in the future
...
const c=a-Date.now();if(0>c||33e5<c){info("authentication session has expired",c);// If we've expired OR if the WSO2 five-minute grace period was not added, then trigger a refresh.
 ...
 */async function configure(a){const b=window[GLOBAL_CONFIG_KEY],c=Object.assign({issuer:DEFAULT_ISSUER,callbackUrl:`${location.origin}${location.pathname}`,autoRefreshOnTimeout:!1},b,a);if(!c.clientId)throw new Error("clientId must be specified in config");const d=new ImplicitGrantProvider(c,window,document);return d.startup()}export{DEFAULT_ISSUER,GLOBAL_CONFIG_KEY,configure};
