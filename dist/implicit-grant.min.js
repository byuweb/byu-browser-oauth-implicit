 ...
 *    See the License for the specific language governing permissions and
 *    limitations under the License.
 */const STORED_STATE_LIFETIME=300000,IG_STATE_REFRESH_REQUIRED="implicit-grant-refresh-required",IG_STATE_AUTO_REFRESH_FAILED="implicit-grant-auto-refresh-failed";// 5 minutes
class ImplicitGrantProvider{constructor(a,b,c,d=new StorageHandler){this.config=a,this.window=b,this.document=c,this.storageHandler=d,this._listeners={},this.store=Object.freeze({state:STATE_INDETERMINATE,user:null,token:null,error:null})}_changeState(a,b,c,d){this.store=Object.freeze({state:a,user:b,token:c,error:d}),this._maybeUpdateStoredSession(a,b,c),_dispatchEvent(this,EVENT_STATE_CHANGE,this.store),this.window.opener&&(this.window.opener.document.dispatchEvent(new CustomEvent("byu-browser-oauth-state-changed",{detail:{state:STATE_AUTHENTICATED,token:c,user:b}})),this.window.close()),this._checkRefresh(c.expiresAt.getTime());// If we're inside the "refresh" iframe,
// then delete now that authentication
// is complete
const e=parent.document.getElementById("byu-oauth-implicit-grant-refresh-iframe");e&&e.parentNode.removeChild(e)}async startup(){this.listen(),this._changeState(STATE_INDETERMINATE);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){this._changeState(STATE_AUTHENTICATING);try{const{state:c,user:d,token:e,error:f}=await _handleAuthenticationCallback(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){console.error("OAuth Error",a),this._changeState(STATE_ERROR,void 0,void 0,a)}}this.hasStoredSession()?this._updateStateFromStorage():this._changeState(STATE_UNAUTHENTICATED)}_checkRefresh(a){var b=this;// Simply using setTimeout for an hour in the future
 ...
 */async function configure(a){const b=window[GLOBAL_CONFIG_KEY],c=Object.assign({issuer:DEFAULT_ISSUER,callbackUrl:`${location.origin}${location.pathname}`,requireAuthentication:!1,displayType:"window"},b,a);if(!c.clientId)throw new Error("clientId must be specified in config");const d=new ImplicitGrantProvider(c,window,document);return d.startup()}export{DEFAULT_ISSUER,GLOBAL_CONFIG_KEY,configure};
