...
const e=parent.document.getElementById(CHILD_IFRAME_ID);return e?d?void 0:(debug("dispatching event to parent"),_dispatchEvent(parent,EVENT_STATE_CHANGE,{state:a,token:c,user:b,source:"iframe"}),void(a===STATE_AUTHENTICATED&&(info("removing child iframe"),e.parentNode.removeChild(e)))):void(this._maybeUpdateStoredSession(a,b,c),a===STATE_AUTHENTICATED&&this._checkExpired(c.expiresAt.getTime()))}async startup(){info("starting up"),this.listen(),this._changeState(STATE_INDETERMINATE);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){debug("handling authentication callback"),this._changeState(STATE_AUTHENTICATING);try{const{state:c,user:d,token:e,error:f}=await _handleAuthenticationCallback(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){error("OAuth Error",a),this._changeState(STATE_ERROR,void 0,void 0,a)}}this.hasStoredSession()?(debug("Has stored session"),this._updateStateFromStorage()):(debug("no authentication present"),this._changeState(STATE_UNAUTHENTICATED))}_checkExpired(a){var b=this;debug("checking expiration time");const c=a-Date.now(),d=0>c,e=c>FIFTY_FIVE_MINUTES_MILLIS;return d||e?void(this.config.autoRefreshOnTimeout?(e&&debug("silently refreshing token to work around odd identity server issue"),this._changeState(STATE_REFRESHING,this.store.user,this.store.token),info("scheduling auto-refresh"),this._schedulePeriodic(function(){return b.startRefresh("iframe")})):d&&this._changeState(STATE_EXPIRED,this.store.user,this.store.token)):void this._scheduleExpirationCheck(a)}_scheduleExpirationCheck(a){var b=this;this._schedulePeriodic(function(){return b._checkExpired(a)})}_schedulePeriodic(a){// Simply using setTimeout for an hour in the future
