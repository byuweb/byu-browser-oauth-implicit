...
if("popup"===a)return void this.window.open(g);let h=document.getElementById(U);h&&h.parentNode.removeChild(h),h=document.createElement("iframe"),h.onload=function(){let a=null;try{a=h.contentWindow.document.body.innerHTML}catch(a){// intentional do-nothing
   ...
   */async function(a){const b=window[_],c=Object.assign({issuer:"https://api.byu.edu",callbackUrl:`${location.origin}${location.pathname}`,doNotAutoRefreshOnTimeout:!1},b,a);if(!c.clientId)throw new Error("clientId must be specified in config");const d=new V(c,window,document);return d.startup()},a}({});
