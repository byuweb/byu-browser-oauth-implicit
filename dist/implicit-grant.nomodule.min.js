...
return d?void 0:(l(this.window.opener,w,{state:a,token:c,user:b,source:"popup"}),void(a===D&&this.window.close()));// Pass event along to parent
// If we're inside the "refresh" iframe
const e=this.window.parent.document.getElementById(U);return e?d?void 0:(l(this.window.parent,w,{state:a,token:c,user:b,source:"iframe"}),void(a===D&&e.parentNode.removeChild(e))):void(this._maybeUpdateStoredSession(a,b,c),a===D&&this._checkRefresh(c.expiresAt.getTime()))}async startup(){this.listen(),this._changeState(B);const a=this._location,b=this._hashParams;if(this.isAuthenticationCallback(a.href,b)){this._changeState("authenticating");try{const{state:c,user:d,token:e,error:f}=await m(this.config,a,b,this.storageHandler);this._changeState(c,d,e,f)}catch(a){console.error("OAuth Error",a),this._changeState("error",void 0,void 0,a)}}this.hasStoredSession()?this._updateStateFromStorage():this._changeState(C)}_checkRefresh(a){var b=this;// Simply using setTimeout for an hour in the future
...
if("popup"===a)return void this.window.open(g);let h=this.document.getElementById(U);h&&h.parentNode.removeChild(h),h=this.document.createElement("iframe"),h.onload=function(){let a=null;try{a=h.contentWindow.document.body.innerHTML}catch(a){// intentional do-nothing
   ...
   */async function(a){const b=window[_],c=Object.assign({issuer:"https://api.byu.edu",callbackUrl:`${location.origin}${location.pathname}`,autoRefreshOnTimeout:!1},b,a);if(!c.clientId)throw new Error("clientId must be specified in config");const d=new V(c,window,document);return d.startup()},a}({});
